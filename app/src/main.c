...
static void sensor_sim_fn(void *p1, void *p2, void *p3)
{
    for (int i = 0; i < EVENT_COUNT; i++) {
        ...
        for (int burst=0; burst<5; burst++) {
            total_events++;
            LOG_INF("[SENSOR] burst %d  tick=%u", total_events, k_uptime_get_32());

            int ret = k_work_reschedule(&debounce_work, K_MSEC(20));
...
int main(void)
{
    ...
    */
    LOG_INF("Sensor events serviced by workqueue handler function");
    LOG_INF("Expected one handler call per single sensor event");

    /* Wait long enough for all events to complete */
    k_msleep((EVENT_COUNT + 2) * SENSOR_MS + 500);

    LOG_INF("Sensor events: %u, workqueue handler calls: %u", total_events, total_processed);
    if (total_events == total_processed) { LOG_WRN("No wasted calls"); }
    else if (total_events > total_processed) { LOG_WRN("Handler called less frequently than sensor's events (is it debouncing?)"); }
    else { LOG_ERR("Wasting CPU time by calling handler more frequently than it has to be!"); }
